import { data } from '@/mock/conversation';
...
const Conversation = {
  ...
  actions: {
    ...
    /* --------------------拉取群组详情--------------------- */
    getGroupInfo: async ({ commit }, params) => {
      let { groupIdList } = params;
    ...
    /* --------------------拿到要更新的lastMsg--------------------- */
    getToDoUpdateLastMsg: ({ commit }, parmas) => {
      ...
      if (parmas.from === myId) {
        ...
        let key =
          parmas.chatType === ('groupChat' || 'chatRoom')
            ? parmas.to
            : parmas.from;
        commit('UPDATE_CONVERSATION_LIST', {
          key,
